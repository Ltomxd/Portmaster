      ...
      <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:10, overflow:'hidden' }}>
        <table style={{ width:'100%', borderCollapse:'collapse' }}>
          <thead><tr style={{ background:'var(--surface2)', borderBottom:'1px solid var(--border)' }}>
            {['ID',T('name'),'PID',T('status'),T('cpu'),T('memory'),T('restarts'),T('uptime'),T('port'),T('actions')].map(h=>(
          ...
          <tbody>
            {processes.length === 0
              ? <tr><td colSpan={10} style={{ padding:40, textAlign:'center', color:'var(--muted)' }}>{T('no_pm2')}</td></tr>
              : processes.map(p => {
                  const c = sc[p.status]??'var(--muted)'
                  return (
                    <tr key={p.id} style={{ borderBottom:'1px solid var(--border)' }}
                      onMouseEnter={e=>(e.currentTarget as HTMLElement).style.background='rgba(255,255,255,.02)'}
                      onMouseLeave={e=>(e.currentTarget as HTMLElement).style.background='transparent'}
                    >
                      <td style={{ padding:'11px 14px', color:'var(--muted)' }}>{p.id}</td>
                      ...
                      <td style={{ padding:'11px 14px' }}><span style={{ display:'inline-flex', alignItems:'center', gap:5, fontSize:12, color:c }}><span style={{ width:6, height:6, borderRadius:'50%', background:c, display:'inline-block', animation:p.status==='online'?'pulse 2s infinite':'none' }} />{p.status}</span></td>
                      <td style={{ padding:'11px 14px', fontSize:12 }}>{p.cpu?.toFixed(1)??0}%</td>
                      <td style={{ padding:'11px 14px', fontSize:12 }}>{fmtMem(p.memory)}</td>
                      <td style={{ padding:'11px 14px', fontSize:12, color:p.restarts>5?'var(--yellow)':'var(--muted)' }}>{p.restarts}</td>
                      <td style={{ padding:'11px 14px', fontSize:12, color:'var(--muted)' }}>{fmtUp(p.uptime)}</td>
                      <td style={{ padding:'11px 14px' }}>{p.port?<span style={{ color:'var(--red2)', fontSize:12, fontFamily:'var(--mono)' }}>{p.port}</span>:'—'}</td>
                      <td style={{ padding:'11px 14px', display:'flex', gap:5 }}>
                        {p.status==='online'
                          ? <><Btn label={T('restart')} v="restart" onClick={()=>onAction(p.name,'restart')} /><Btn label={T('stop')} v="stop" onClick={()=>onAction(p.name,'stop')} /></>
                          : <Btn label={T('start')} v="start" onClick={()=>onAction(p.name,'start')} />}
                      </td>
                    </tr>
                  )
              })
