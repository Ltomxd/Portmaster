...
export function ToastList({ toasts, onDismiss }: { toasts:Toast[]; onDismiss:(id:number)=>void }) {
  ...
  return (
    <div style={{ position:'fixed', bottom:12, right:12, left:12, zIndex:300, display:'flex', flexDirection:'column', gap:8 }}>
      {toasts.map(t => (
        <div key={t.id} onClick={()=>onDismiss(t.id)} style={{
          background:'var(--surface)', borderRadius:8, padding:'10px 16px', fontSize:13, minWidth:220, maxWidth:'100%',
