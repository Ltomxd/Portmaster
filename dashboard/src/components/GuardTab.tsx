      ...
      {entries.length === 0 ? (
        <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:10, padding:40, textAlign:'center' }}>
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:12 }}>
            ...
            <div style={{ background:'var(--surface2)', border:'1px solid var(--border2)', borderRadius:8, padding:'10px 20px' }}>
              <code style={{ color:'var(--red2)', fontSize:12 }}>portmaster guard --port 3000 --auto-kill</code>
            </div>
          </div>
        </div>
      ) : (
          ...
          {entries.map(([key, g]) => (
            <div key={key} style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:10, padding:20 }}>
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14 }}>
                <span style={{ fontWeight:600, fontSize:14 }}>Guard: {key}</span>
                <span style={{ fontSize:11, padding:'3px 10px', borderRadius:10, fontWeight:500,
                  background: g.running?'var(--red-glow)':'var(--surface2)',
                  border: g.running?'1px solid rgba(229,62,62,.3)':'1px solid var(--border)',
                  color: g.running?'var(--red2)':'var(--muted)' }}>
                  {g.running ? `● ${T('active')}` : `○ ${T('inactive')}`}
                </span>
              </div>
              <div style={{ maxHeight:200, overflowY:'auto' }}>
                {(g.recentEvents??[]).length === 0
                  ? <div style={{ color:'var(--muted)', fontSize:12 }}>{T('no_events')}</div>
                  : [...g.recentEvents].reverse().slice(0,10).map((ev: GuardEvent, i) => (
                    <div key={i} style={{ display:'flex', gap:10, padding:'5px 0', borderBottom:'1px solid var(--border)', fontSize:12, alignItems:'center' }}>
